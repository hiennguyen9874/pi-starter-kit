---
name: design-qa
description: "Internal prototype QA helper. Use only after an image-to-code build has a source visual target and a rendered implementation to compare before handoff."
---
...
- a source visual target: exported design frame, image, screenshot, mockup, or source capture
...
1. Identify the comparison target.
   - Determine the source design: exported frame, image, design board, screenshot, spec, or mockup.
...
2. Capture evidence.
   - For Figma or other design URLs, ask the user for an exported PNG/frame; Pi has no Codex design-connector. Compare the exported image file, never an unauthenticated URL.
   ...
   - For web/app implementations, follow the Browser Choice rule in [index](../product-design-router/SKILL.md#browser-choice-pi). Start the local dev server, `open` the local URL with `agent_browser`, then capture `screenshot` at the intended viewport.
...
For Pi builds, `design-qa.md` must include the `agent_browser`-rendered implementation screenshot, viewport, primary interactions tested, console errors checked, and final result. If browser-rendered evidence is missing, `final result` is `blocked`.
