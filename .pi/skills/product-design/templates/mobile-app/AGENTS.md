...
Run the local server yourself (e.g. `npm run dev -- --host 127.0.0.1 --port 5173`), open the local URL with the `agent_browser` tool (`open` → `snapshot -i` → `screenshot`), and verify the rendered app and its primary interactions. Hand off the clickable local URL. Do not give the user server-start instructions when you can run it.
...
- `npm run build` preserves the mobile runtime and prepares the static output under `dist/client` plus `dist/server/index.js`. The legacy Sites files (`.openai/hosting.json`, `npm run test:sites`) are optional on Pi; only check them for an explicit Sites handoff.
