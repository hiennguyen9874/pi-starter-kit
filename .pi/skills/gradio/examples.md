...
## Transformers Text Generation Interface
...
demo = gr.Interface(
    fn=predict,
    inputs=gr.Textbox(lines=4, label="Prompt", placeholder="My favorite programming language is"),
...
def classify_image(image):
    return {"cat": 0.75, "dog": 0.2, "rabbit": 0.05}
...
demo = gr.Interface(
    ...
    title="Toy Classification Scores",
    live=True,
)
...
## Hugging Face Hub or Space Loader

```python
import gradio as gr

# Requires a Gradio version that supports Interface.load().
gr.Interface.load(
...
# Spaces can also be loaded and customized.
gr.Interface.load(
...
## Blocks Multi-Step Speech Sentiment
...
with gr.Blocks() as demo:
    audio_file = gr.Audio(type="filepath", label="Speech")
    text = gr.Textbox(label="Transcript")
    label = gr.Label(label="Sentiment")

    recognize = gr.Button("Recognize Speech")
    classify = gr.Button("Classify Sentiment")

    recognize.click(speech_to_text, inputs=audio_file, outputs=text)
    classify.click(text_to_sentiment, inputs=text, outputs=label)
