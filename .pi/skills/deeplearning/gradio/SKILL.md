---
name: gradio
description: Use when creating or editing Gradio Python apps, ML demos, Hugging Face Spaces, Interface/Blocks layouts, components, event listeners, stateful demos, streaming demos, or chatbot UIs.
---

# Gradio

Gradio builds interactive web demos around ordinary Python functions. It is model-agnostic: use it for Transformers pipelines, PyTorch/TensorFlow models, audio/image/text utilities, or any function whose inputs and outputs can be represented by Gradio components.

## Choose the API

- Use `gr.Interface` for a single prediction function with clear input and output components.
- Use component classes like `gr.Textbox(...)`, `gr.Audio(...)`, or `gr.Image(...)` instead of string shortcuts when labels, placeholders, sources, types, examples, or layout details matter.
- Use `gr.Blocks` when the app needs custom layout, tabs, multiple steps, chained data flow, dynamic component properties, or multiple independent actions.
- Use `gr.ChatInterface` for a standard chat UI before hand-rolling a chatbot in `Blocks`.

## Interface Pattern

`Interface(fn, inputs, outputs, ...)` maps each input component to a function parameter in order, and each returned value to an output component in order.
...
demo = gr.Interface(
    fn=predict,
    inputs=gr.Textbox(label="Prompt", lines=4, placeholder="Start a story..."),
    outputs=gr.Textbox(label="Completion"),
    title="Text Generation Demo",
    description="Generate text with a Transformers pipeline.",
    examples=[["My favorite programming language is"]],
)

demo.launch()
```

Useful `Interface` options from the Hugging Face course:

- `title`, `description`, and `article` polish the demo for non-technical users.
- `examples` should be a list of samples; each sample is a list matching the input component order.
- `live=True` reruns the function whenever inputs change. Use it only for fast functions.
- `interpretation="default"` or `interpretation="shap"` can expose which input regions influenced a prediction when supported.

## Common ML Component Patterns

```python
# Audio as numpy: function receives (sample_rate, data) and can return the same shape.
...
# Audio as filepath: best for Transformers ASR pipelines.
asr = pipeline("automatic-speech-recognition")

def transcribe(audio_path):
    return asr(audio_path)["text"]

gr.Interface(fn=transcribe, inputs=gr.Audio(type="filepath"), outputs="text")

# Label output: return {label: probability} for ranked classification results.
def classify(image):
...
- `demo.launch()` serves locally, usually at `http://localhost:7860`.
- `demo.launch(inbrowser=True)` opens a local browser tab.
- `demo.launch(share=True)` creates a temporary public link routed to the local process. Treat it as public: do not expose secrets, destructive functions, private files, or privileged local actions.
- For permanent hosting, create a Hugging Face Space with an `app.py` containing the Gradio app and required dependency files.

## Hugging Face Hub and Spaces

The course shows `gr.Interface.load()` for loading Hub models and Spaces into a local Gradio interface. Use it when the installed Gradio version supports it; otherwise prefer the current Gradio/Hugging Face client APIs.

```python
import gradio as gr

# Model via Hugging Face Inference API. Useful when the model is too large to load locally.
...
# Recreate a Space locally and override selected interface options.
gr.Interface.load(
...
`Blocks` gives explicit control over layout and data flow. Components render in creation order; `Row`, `Column`, `Tabs`, `Tab`, and `Accordion` structure the page. Events connect component values to Python functions.
...
with gr.Blocks() as demo:
    ...
    flip_btn.click(lambda text: text[::-1], inputs=input_text, outputs=output_text)

demo.launch()
```

Use the same component as an output of one event and input to another for multi-step demos:

```python
with gr.Blocks() as demo:
    audio = gr.Audio(type="filepath")
    text = gr.Textbox(label="Transcript")
    sentiment = gr.Label(label="Sentiment")

    transcribe_btn = gr.Button("Recognize Speech")
    classify_btn = gr.Button("Classify Sentiment")

    transcribe_btn.click(speech_to_text, inputs=audio, outputs=text)
    classify_btn.click(text_to_sentiment, inputs=text, outputs=sentiment)
```

## State

State persists within one user's page session; it is not shared across users. In `Interface`, add an extra state argument and return value. In `Blocks`, prefer `gr.State`.
...
iface = gr.Interface(
    ...
    outputs=[gr.Chatbot(type="messages"), gr.State([])],
)
iface.launch()
```
